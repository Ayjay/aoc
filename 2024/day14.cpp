...
auto parse(std::string_view s) {
    return *bp::parse(s, *("p=" > bp::long_long > ',' > bp::long_long > " v=" >
                           bp::long_long > ',' > bp::long_long > -bp::eol));
}

enum quad { top_left, bottom_left, top_right, bottom_right };
...
auto run_b(std::string_view s) {
    ...
    for (auto i : rv::iota(1ll)) {
        ...
        auto tiles = tiles_ | rv::keys | ranges::to<boost::unordered_set> |
                     ranges::to<std::vector>;
        const auto get_y = [](auto v) { return std::get<1>(v); };
        ranges::sort(tiles, std::less{}, get_y);
        auto rows = tiles | rv::chunk_by([](auto p, auto q) {
                        return std::get<1>(p) == std::get<1>(q);
                    });
        if (ranges::any_of(rows, [](auto subrange) {
                return ranges::distance(subrange) > 30;
            })) {
