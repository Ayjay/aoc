...
static auto run_b(std::string_view s) {
    ...
    const auto antinodes_from_pair = [&](const auto ps) {
        const auto [a,b] = ps;
        const auto diff = b - a;
        const auto a_backwards = rv::iota(0) | rv::transform([=](auto i) { return a - diff * i; }) | rv::take_while(on_map);
        const auto b_forwards = rv::iota(0) | rv::transform([=](auto i) { return b + diff * i; }) | rv::take_while(on_map);
    ...
    const auto antinodes = [&](const auto& as) {
        ...
        return rv::cartesian_product(as, as)
            | rv::filter(less_than)
            | rv::transform(antinodes_from_pair)
            | rv::join;
    };
