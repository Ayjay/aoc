...
TEST_CASE("antinode pair", "[day8]") {
    REQUIRE(antinode_pair(std::tuple{vector2{3, 4}, vector2{5, 5}}) ==
...
const auto antinodes = [](const auto& as) {
    ...
    return rv::cartesian_product(as, as) | rv::filter(less_than) |
           rv::transform(antinode_pair) | rv::join;
};

TEST_CASE("antinodes", "[day8]") {
    REQUIRE(ranges::equal(antinodes(std::vector{vector2{3, 4}, vector2{5, 5}}),
                          std::array{vector2{1, 3}, vector2{7, 6}}));
}
}  // namespace a
...
auto run_a(std::string_view s) {
    ...
    auto all_antinodes = antennae | rv::values | rv::transform(a::antinodes) |
                         rv::join | rv::filter(on_map) |
...
static auto run_b(std::string_view s) {
    ...
    const auto antinodes_from_pair = [&](const auto ps) {
        const auto [a, b] = ps;
        const auto diff = b - a;
        const auto a_backwards =
            rv::iota(0) | rv::transform([=](auto i) { return a - diff * i; }) |
            rv::take_while(on_map);
        const auto b_forwards =
            rv::iota(0) | rv::transform([=](auto i) { return b + diff * i; }) |
            rv::take_while(on_map);
        return rv::concat(a_backwards, b_forwards);
    };
    ...
    const auto antinodes = [&](const auto& as) {
        ...
        return rv::cartesian_product(as, as) | rv::filter(less_than) |
               rv::transform(antinodes_from_pair) | rv::join;
    };

    auto all_antinodes = antennae | rv::values | rv::transform(antinodes) |
