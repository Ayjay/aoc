...
const auto test_data =
    std::vector{std::tuple<std::string_view,
                           ...
                           std::optional<result_type>>{R"(0123
1234
8765
9876)",
                                                       1,
...
auto run_a(std::string_view s) {
    ...
    auto score = [&](this auto& self,
                     vector2 p) -> boost::unordered_set<vector2>& {
        ...
        if (g.get(p) == '9') {
            ...
            auto step_ups = directions | rv::transform(get_adjacent) |
                            rv::filter(is_step_up);
            result = ranges::accumulate(step_ups, std::move(result),
                                        combine_sets, self);
        }
    ...
    auto zeros =
        g.cells() | rv::filter([&](auto p) { return g.get(p) == '0'; });

    return reduce(zeros | rv::transform(score) |
                  rv::transform([](const auto& s) { return s.size(); }));
}
